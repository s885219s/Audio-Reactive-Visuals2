...
(function() {
  ...
  Silk = (function() {
    ...
    Silk.prototype.frame = function() {
      ...
      this.frameTime++;//註銷掉 粒子變多
      //console.log("up:"+this.frameTime);//這個點下去就重算
      _results = [];
    ...
    Silk.prototype.drawCurve = function(instr) {
      ...
      //console.log(ctx.globalAlpha);//0.00多~0.0多一點 而已
      ctx.beginPath(); //註銷掉 變成線條分明 還會失去顏色
      ctx.moveTo(curve[0].x, curve[0].y);//moveto起始點 lineto結束點
      p1 = curve[1];
  ...
  PointPreviewSilk = (function(_super) {
    ...
    function PointPreviewSilk(ctx, scaleInfo, state) {
      ...
      if (this.previewOpacity == null) {
        this.previewOpacity = 1;//opacity 不透明的意思
      }
    ...
    /*
     ...
     if (p.age < p.lifespan) {
     ctx.beginPath();
     ctx.globalAlpha = p.a * (1 - p.age / p.lifespan);//消失 fade out
     if (p.invertA) {
    ...
    PointPreviewSilk.prototype.drawCurve = function(instr) {
      var fillStyle, p;
      p = this.curve[0];
      this.ctx.beginPath();//如果這行註銷掉，中間intro沒跑完，他不會消失
      fillStyle = this.ctx.fillStyle;
      this.ctx.fillStyle = '#4e4866';
      if (typeof this.previewEmphasisFn === "function" ? this.previewEmphasisFn(instr) : void 0) {
        this.ctx.fillStyle = '#ffffff';
      }
      this.ctx.arc(p.x, p.y, instr.scale * this.previewRadius, 0, 2 * Math.PI, false);//arc(x,y,radius,startangle,endangle,true/false)
      this.ctx.fill();
      this.ctx.fillStyle = fillStyle;
      this.ctx.globalCompositeOperation = 'source-over';//在原有圖形上畫新圖像，覆蓋原有圖像
      this.ctx.strokeWidth = 0.5;//改數值沒有差...
      this.ctx.stroke();
  ...
  this.Silks = (function() {
    function Silks(container, silkCanvas, bufferCanvas, sparksCanvas) {
      ...
      this.silkSettingsState = $.extend(true, {}, Silk.initialState);//extend會直接以後方obj複寫前方的obj
      this.fillSilkCanvas();
    ...
    Silks.prototype.frame = function(dt) {
      ...
      //console.log("down:"+this.frameTime);
      //]\if(this.frameTime%20===0){  //消除筆畫
      //}
    ...
    Silks.prototype.disappear = function(canvas) {
    ...
    Silks.prototype.clearSilkCanvas = function(canvas) {
      var ctx;
      ctx = canvas.getContext('2d');
      ctx.globalCompositeOperation = 'source-over';
      ctx.globalAlpha = 1;
    ...
    Silks.prototype.initInputEvents = function() {
      var fadeTimer, previewActive,canvas;
      ...
      //$(this.sparksCanvas).mousemove((function(_this) {
      //  return function(e) {
      //    if(!_this.inputIsActive) {
      //      _this.updateInputFromEvent(e.pageX, e.pageY);
      //      _this.stopPreviewingInput();
      //      _this.inputStarted();
      //      return false;
      //    }
      //    else {
      //      _this.updateInputFromEvent(e.pageX, e.pageY);//線條會畫向滑鼠位置
      //    }
      ...
      var audioContext,
          soundSource,
          soundBuffer,
          url = 'Silk – Interactive Generative Art_files/Period.mp3';
      var audioInput = null,
          realAudioInput = null,
          inputPoint = null,
          analyserNode = null,
      ...
      this.useMic=false;
      var _this=this;//this=Silk{} 但是傳進去後的this=Windows{}
      ...
      document.addEventListener('dragover', dropOver, false);
      ...
      function dropEvent(evt) {
        loading=true;
        beat=false;
        _this.inputIsActive=false;
        _this.clearSilkCanvas(_this.silkCanvas);
        evt.stopPropagation();
        evt.preventDefault();
        if (_this.isPlayingAudio) {
          stopSound();
      ...
      function change(){
        ...
        switch(x) {
          case 0:
            ...
            _this.silkSettingsState.highlightColor="#717171";
            break;
          case 1:
            _this.silkSettingsState.color="#276f9b";
            _this.silkSettingsState.highlightColor="#3dbbea";
            break;
          case 2:
            _this.silkSettingsState.color="#65eb43";
            _this.silkSettingsState.highlightColor="#53BD39";
            break;
          case 3:
            _this.silkSettingsState.color="#fdf23f";
            _this.silkSettingsState.highlightColor="#E3BF30";
            break;
          case 4:
            _this.silkSettingsState.color="#fd722e";
            _this.silkSettingsState.highlightColor="#EB5126";
            break;
          case 5:
            _this.silkSettingsState.color="#dd4876";
            _this.silkSettingsState.highlightColor="#fe495e";
            break;
          case 6:
            _this.silkSettingsState.color="#825ba1";
            _this.silkSettingsState.highlightColor="#b585da";
            break;
        }
        x = Math.floor((Math.random() * 7));
        switch(x) {
          case 0:
            _this.silkSettingsState.symNumRotations=6;
            break;
          case 1:
            _this.silkSettingsState.symNumRotations=4;
            break;
          case 2:
            _this.silkSettingsState.symNumRotations=6;
            break;
          case 3:
            _this.silkSettingsState.symNumRotations=5;
            break;
          case 4:
            _this.silkSettingsState.symNumRotations=6;
            break;
        ...
        switch(x) {
          case 0:
            _this.silkSettingsState.spiralCopies= 1;
            break;
          case 1:
            _this.silkSettingsState.spiralCopies= 4;
            break;
          case 2:
            _this.silkSettingsState.spiralCopies= 4;
            break;
        ...
        switch(x) {
          case 0:
            _this.silkSettingsState.symMirror =true;
            break;
          case 1:
            _this.silkSettingsState.symMirror =true;
            break;
          case 2:
            _this.silkSettingsState.symMirror =false;
            break;
          case 3:
            _this.silkSettingsState.symMirror =true;
            break;
        }
        //console.log(_this.silkSettingsState.color+','+_this.silkSettingsState.highlightColor+','+_this.silkSettingsState.symNumRotations+','+_this.silkSettingsState.spiralCopies+','+_this.silkSettingsState.symMirror);

      }
      function onBeat(canvas){
        console.log("beat:"+_this.frameTime);
        beat = true;
        beatcount=0;
        _this.inputIsActive = false;
        _this.clearSilkCanvas(canvas);
        change();
        setTimeout(function(){beat = false;},50);
        //_this.updateInputFromEvent(-canvas.width*magnitude,-canvas.height*magnitude);
      }
        ...
        navigator.getUserMedia(
            {
              ...
              micsoundSource = audioContext.createBufferSource();
      ...
      function updateAnalysers(time) {
        var freqByteData = new Uint8Array(analyserNode.frequencyBinCount);//analyserNode.frequencyBinCount=1024
        analyserNode.getByteFrequencyData(freqByteData);
        var multiplier = analyserNode.frequencyBinCount;

        var micfreqByteData = new Uint8Array(micanalyserNode.frequencyBinCount);//analyserNode.frequencyBinCount=512
        micanalyserNode.getByteFrequencyData(micfreqByteData);
        var micmultiplier = micanalyserNode.frequencyBinCount;
        //console.log(micfreqByteData);
        ...
        //var ac = autoCorrelate(timeByteData , audioContext.sampleRate );
        //
        //pitch = ac;
        ...
        magnitude = magnitude / multiplier/256;
        for (var j = 0; j< micmultiplier; j++) {
          micmagnitude += micfreqByteData[j];
        }
        micmagnitude = micmagnitude / micmultiplier/256;

        magnitude=Math.floor(magnitude*100)/100;
        //console.log(_this.frameTime);
        //console.log(w*_this.frameTime);
        //console.log((canvas.width/2)*(Math.cos(w*_this.frameTime)));
        ...
        //  _this.silkSettingsState.color="#555555";
        //  _this.silkSettingsState.highlightColor="#717171"
        //  console.log(_this.silkSettingsState.color);
        //  console.log(_this.silkSettingsState.symNumRotations);
        //}
        //console.log(magnitude);
        //console.log(canvas.width,canvas.height);

        //傳座標畫圖
        if(!_this.inputIsActive&&!beat&&!loading) {
          //console.log("start:"+_this.frameTime);
          _this.updateInputFromEvent(x,y);
          _this.stopPreviewingInput();
          _this.inputStarted();
        }
        else if(_this.inputIsActive){
          //console.log("going:"+_this.frameTime);
          //if(_this.frameTime%10==0) {
           // console.log(magnitude);
            _this.updateInputFromEvent(x,y);//線條會畫向滑鼠位置
          //}
        }
        else{
          //console.log("loading");
      ...
      /*$(this.sparksCanvas).mousedown((function(_this) {
        return function(e) {
          if (e.button === 2) {
            console.log("twice");//botton=2 是右鍵
            return;
          }
          _this.updateInputFromEvent(e);//線條會畫向滑鼠位置(白點會跟著滑鼠)
          _this.stopPreviewingInput();//讓預覽位置的黑點點不見
          _this.inputStarted();//作畫的源頭
          return false;//沒什麼作用
        };
       ...
       _this.inputEnded();//讓inputactive=false
       }
       return previewActive();
       };
       })(this))*/
      //console.log(this); 出現整個silk的參數資訊
      /*$(this.sparksCanvas).mouseenter((function(_this) {
        return function(e) {
      ...
      /*$(this.sparksCanvas).bind('touchstart', (function(_this) {
        return function(e) {
          var _ref1;
      ...
      return function() {
        ...
        //  getMicInput();
        //}
        //else if(_this.useMic){
        //  initAudio();
        //}
        //else if(!_this.isPlayingAudio&&!_this.useMic) {
        //  playSound();
        //}
    ...
    Silks.prototype.inputStarted = function() {
      ...
      return this.activeSilkId = this.add();//呈現圖形
    };
    ...
    //讓圖形隨著滑鼠移動
    Silks.prototype.updateInputFromEvent = function(x,y) {
      //e.pageX是滑鼠游標的座標
      //console.log(this.container.offsetLeft); 都是零，不知為
      this.inputX = x - this.container.offsetLeft;
      return this.inputY = y - this.container.offsetTop;
      //console.log(e.pageX);
      //console.log(magnitude);
      //this.inputX = magnitude*10;
      //return this.inputY = magnitude*10;
    };
  ...
  */
  ...
  initUI = function(silks) {
    ...
    initColorPicker = function() {
      ...
      _ref1 = colors.slice(0, 6);//stringObject.slice(start,end)題取字串的部分 //_ref1=colors
      ...
      //console.log(swatches);//objects with many functions
      ...
      return $(window).on('mousemove', function(e) {
        ...
        if (draggedSwatch != null) {
          ...
          silks.silkSettingsState.color = draggedSwatch.trueColor();//顏色變換的code
          ...
          //console.log(silks.silkSettingsState.color+','+silks.silkSettingsState.highlightColor);
          if (typeof Hue !== "undefined" && Hue !== null) {
    ...
    symSlider = initSlider('#sym-num-rotations', {
      ...
      if (!ghost) {
        silks.extendSilkSettingsState(state);//不影響顏色切換
      }
    ...
    return function() {
      ...
      hideIntroSilk = function() {
        ...
        if (b.introSilkShowing()) {
          silks.clear(false);
          //silks.clearSilkCanvas(silkCanvas);
          return b.introSilkShowing(false);
        }
      };
      //一開始程式自製的圖形
      //$('#sparks').mousedown(hideIntroSilk);//只是讓一開始的intro圖形消失而已
      window._s = silks = new Silks(container, silkCanvas, bufferCanvas, sparksCanvas);
      silks.initInputEvents();
      //silks.clearSilkCanvas(silks.silkCanvas);//silks= audio的_this= Silks{}
      ui = initUI(silks); //沒他 一開始跑不動
      startTime = +new Date();
          ...
          switch (false) {
            case frameCount !== introStart:
              introSilkId = silks.add('intro', {
                symNumRotations: 6,
                symMirror: true,
                color: '#276f9b',//#276f9b ['#3e95cc', '#82cefc', '#276f9b', '#3dbbea']
                highlightColor: '#3dbbea',//#825ba1  dd4876
                highlightMode: 'time',
      ...
      $('#sparks').mousedown(function(e) {
        //console.log(e.pageX+','+e.pageY);//windows{}
        if(silks.audio) {
          pmouseX = e.pageX;
          pmouseY = e.pageY;
          b.silkActive(true);
